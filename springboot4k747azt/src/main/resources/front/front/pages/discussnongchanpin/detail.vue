		...
		<view class="container" :style='{"width":"100%","padding":"20rpx","position":"relative","background":"#DCF7FF","height":"auto"}'>
			<view :style='{"width":"100%","padding":"0","background":"none","height":"auto"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"0","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 1%","color":"rgb(255, 255, 255)","background":"#4EA0B8","width":"calc(100% / 3 - 2%)","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' v-if="userid&&isAuth('discussnongchanpin','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 1%","color":"rgb(255, 255, 255)","background":"#4EA0B8","width":"calc(100% / 3 - 2%)","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('discussnongchanpin','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussnongchanpinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussnongchanpinCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discussnongchanpin', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussnongchanpin');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discussnongchanpin');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussnongchanpin', this.id);
				let reg=new RegExp('http://localhost:8080/springboot4k747azt/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('discussnongchanpin',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussnongchanpin', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscussnongchanpin',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscussnongchanpin/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussnongchanpin',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussnongchanpin',that.detail)
							setTimeout(()=>{
