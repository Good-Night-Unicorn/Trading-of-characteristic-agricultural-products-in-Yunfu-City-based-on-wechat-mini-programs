...
<view class="content">
	<view :style='{"minHeight":"100vh","width":"100%","padding":"0","position":"relative","background":"#DCF7FF","height":"auto"}'>
		<form :style='{"width":"100%","padding":"0","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0 0 2rpx 0","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"26rpx","color":"#727272","textAlign":"right"}' class="title">评分</view>
			...
			async onSubmitTap() {
				...
				//跨表计算
						...
						if(this.ruleForm.id){
							...
							oet = await this.$api.add(`discussnongchanpin`, this.ruleForm);
						}
						uni.setStorageSync('discussnongchanpinCleanType',true);
						let rs2 = await this.$api.list('discussnongchanpin',{page: 1,limit: 10000,refid: this.ruleForm.refid})
						let totalscore = 0
						for(let x in rs2.data.list){
							totalscore += Number(rs2.data.list[x].score)
						}
						let rs3 = await this.$api.info('nongchanpin',this.ruleForm.refid)
						rs3.data.totalscore = Number((totalscore / rs2.data.list.length).toFixed(2))
						await this.$api.update('nongchanpin',rs3.data)
