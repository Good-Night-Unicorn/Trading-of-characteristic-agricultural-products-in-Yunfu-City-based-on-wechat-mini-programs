	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="投诉内容" prop="tousuneirong" >
					<el-input v-model="ruleForm.tousuneirong" placeholder="投诉内容" clearable  :readonly="ro.tousuneirong"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="投诉内容" prop="tousuneirong" >
					<el-input v-model="ruleForm.tousuneirong" placeholder="投诉内容" readonly></el-input>
				</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jiufentousu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiufentousuCrossAddOrUpdateFlag = false;
											this.parent.search();
